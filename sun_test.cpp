...
int main()
{
  ...
  printlogo();
   ...
   //zenith angle sun at N, E (for S or W minus sign)
   //      std::cout<<a(-60,d,13)<<std::endl;       


    //definition state (number flavour, neutrino or antineutrino
...
/*  std::ofstream filekaon("flux_nutau.txt");
 for(double lE=-8.9; lE<=0; lE+=(8.9)/(179-1)){
     filekaon << lE;
     //for(int fl=1;fl<=28;fl++){
     int fl=9;
       filekaon << " " <<  fluxtau.flux_pion(massdm,lE,fl);
       //};
      filekaon << std::endl;                                                                             
 };
*/
...
// std::ofstream file("fluxes_flavor_ch9.txt");
/* 
  int Nen=700;
  int Ncz=100;
  ...
  for(int fl=0; fl<Nneutrinos; fl++){
    file << " " << fluxe.flux_pion(massdm,log10(Einit/massdm),channel)<<" "<<fluxmu.flux_pion(massdm,log10(Einit/massdm),channel)<<" "<<fluxtau.flux_pion(massdm,log10(Einit/massdm),channel) <<" "<<nus_sun.EvalFlavor(fl)<<" ";
  }
  for(int fl=0; fl<Nneutrinos; fl++){
    file<<" "<<nus_vac.EvalFlavor(fl)<<" "; }
  for(int fl=0; fl<Nneutrinos; fl++){
